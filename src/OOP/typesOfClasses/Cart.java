...
public class Cart {
	...
	private BigDecimal calculateTotalNetPrice_2() {
		BigDecimal sum = new BigDecimal(0);
		for (Product product : products ) {
			sum.add(product.getPrice());
		}
		this.totalNetPrice = sum.setScale(MONEY_SCALE, RoundingMode.HALF_UP);
		return this.totalNetPrice;
	}
	
	private BigDecimal calculateTotalNetPrice() {
		double ntPrice = Arrays.stream(this.products)
		  .mapToDouble(product -> product != null ? product.getPrice().doubleValue() : 0)
		  .sum();
		
		this.totalNetPrice = BigDecimal.valueOf(ntPrice).setScale(MONEY_SCALE, RoundingMode.HALF_UP);
