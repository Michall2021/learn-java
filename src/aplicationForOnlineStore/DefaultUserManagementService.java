...
public class DefaultUserManagementService implements UserManagementService{
	...
	private static final String EMPTY_EMAIL_ERROR_MESSAGE = "You have to input email to register. "
	...
	private String checkuniqueEmail(String email) {
		...
		for (User user : users) {
			if (user != null && 
					user.getEmail() != null &&
					user .getEmail().equalsIgnoreCase(email)) return NOT_UNIQUE_EMAIL_ERROR_MESSAGE;
		}
		return NO_ERROR_MESSAGE;
	}
	...
	public User[] getUsers() {
		...
		for (User user : users) {
			if (user != null) nonNullUsersAmount++;
		}
		...
		int index = 0;
		for (User user : users) {
			if (user != null) nonNullUsers[index++] = user;
		}
	...
	public User getUserByEmail(String userEmail) {
		for(User user : users) {
			if (user != null && user.getEmail().equalsIgnoreCase(userEmail)) return user;
